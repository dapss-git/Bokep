...
class Function {
  ...
  useLimit(user, amount = 1) {
    if (!user || typeof user.limit !== "number") {
      return {
        success: false,
        remaining: 0,
      };
    ...
    // Directly modify and ensure proxy catches it
    const oldLimit = user.limit;
    const newLimit = user.limit - amount;
    user.limit = newLimit; // This should trigger the proxy set trap
    
    // Debug logging
    console.log(`[LIMIT] User limit: ${oldLimit} → ${newLimit} (used: ${amount})`);
