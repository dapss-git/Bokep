...
class PluginsLoad {
  ...
  item.isFile() &&
  (p.endsWith(".js") || p.endsWith(".cjs") || p.endsWith(".ts"))
) {
  await this.add(p);
}
  ...
  if (require.cache[pluginPath]) delete require.cache[pluginPath]
  data = require(pluginPath)
  if (data) data.type = "commonjs"
  ...
  async load() {
    ...
    const handleFileChange = async (path) => {
      ...
      if (path.endsWith(".js") || path.endsWith(".cjs") || path.endsWith(".ts")) {
