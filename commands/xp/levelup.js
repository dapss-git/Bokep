    ...
    try {
        ...
        if (!canLevelUp(user[m.sender].level, user[m.sender].exp, 38)) {
            let {
                ...
                xp,
                max
            } = xpRange(user[m.sender].level, 38)
            return m.reply(`
...
`.trim()
            ...
            const avatarURL = await conn.profilePictureUrl(m.sender, 'image').catch(() => 'src/avatar_contact.png')

            const img = await generateLevelUpImage(avatarURL, before, user[m.sender].level, name)
