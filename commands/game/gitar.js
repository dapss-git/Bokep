...
<div class="card">
 <div class="head">
  <div>
   <div class="brand">RAMZ OFFICIAL</div>
   <div class="title">GUITAR HERO</div>
  </div>
  <div class="score">
   <span id="score">000000</span>
   <div class="best">BEST <span id="best">000000</span></div>
  </div>
 </div>

 <div class="info">
  <span>❤️ <b id="life">5</b></span>
  <span>🔥 COMBO <b id="comboTop">0</b></span>
 ...
 <div class="stage" id="stage">
  ...
  <div class="combo" id="combo">READY TO ROCK 🎸</div>
  ...
  <div class="pop" id="pop">PERFECT!</div>
 </div>
 ...
 <div class="bottom">
  TAP THE NOTES • HIT THE BEAT • ROCK THE STAGE 🎸
 </div>
</div>
...
function popup(text){
 ...
 pop.style.color=text==="MISS"?"#ff4f70":"#fff";
 pop.style.opacity="1";
...
function hitLane(lane){
 ...
 if(candidate && distance<48){
  ...
  combo=0;
  comboEl.textContent="KEEP THE BEAT 🎸";
  popup("MISS");
  life--;
  sound(120);
...
function endGame(){
 running=false;
 comboEl.textContent="GAME OVER • TAP TO RESTART";
}
...
function restart(){
 ...
 comboEl.textContent="READY TO ROCK 🎸";
 updateUI();
}
...
`;
    ...
    try {
        const data = Buffer.from(JSON.stringify({
            response_id: "ramz-guitar-hero",
            sections: [{
                view_model: {
                    primitive: {
                        __typename: "GenAIaeacdsnwHtmlPrimitive",
                        payload: html,
                        trusted_sources: [global.ownername || "Ramz Official"]
                    },
...
handler.help = ["guitar"];
handler.tags = ["game"];
handler.command = /^(guitar|guitarhero)$/i;
