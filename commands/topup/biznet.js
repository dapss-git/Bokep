...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*P E M B A Y A R A N  B I Z N E T*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *ID Pelanggan* Biznet.\n\n`
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan pembayaran Biznet sedang tidak tersedia.`)
        }
        ...
        console.error('[BIZNET FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['biznet <nomor>']
handler.tags = ['topup']
handler.command = ['biznet']
