...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk PUBG sedang tidak tersedia.`)
        }
        ...
        console.error('[GAME PUBG FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['topuppubg <id>']
handler.tags = ['topup']
handler.command = ['topuppubg', 'pubg', 'pubgm']
