...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan PDAM sedang tidak tersedia saat ini.`)
        }
...
handler.help = ['pdam', 'air', 'bayarpdam'].map(v => v + ' <nomor_tujuan>')
handler.tags = ['topup']
handler.command = ['pdam', 'air', 'bayarpdam']
