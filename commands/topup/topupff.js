...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Free Fire sedang tidak tersedia.`)
        }
        ...
        console.error('[GAME FF FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['topupff <id>']
handler.tags = ['topup']
handler.command = ['topupff', 'ff', 'epep']
