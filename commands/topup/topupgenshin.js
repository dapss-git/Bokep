...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  G E N S H I N  I M P A C T*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *ID Player* / *Nomor* tujuan.\n\n`
        msg += `*Contoh:* ${usedPrefix + command} 123456789\n`
        return m.reply(msg)
    }
    
    const search = 'genshin'
    ...
    try {
        ...
        let products = data.filter(item => {
            ...
            const isDigital = item.kategori === 'DIGITAL' || item.kategori.includes('GAME') || item.kategori.includes('VOUCHER')
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Genshin Impact sedang tidak tersedia.`)
        }
        ...
        console.error('[GAME FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['topupgenshin', 'genshin'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topupgenshin', 'genshin']
