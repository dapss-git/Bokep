...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*P E M B A Y A R A N  M Y  R E P U B L I C*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *ID Pelanggan* My Republic.\n\n`
        msg += `*Contoh:* ${usedPrefix + command} 123456789\n`
        return m.reply(msg)
    }
    
    const search = 'republic'
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan pembayaran My Republic sedang tidak tersedia.`)
        }
        ...
        console.error('[MYREPUBLIC FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['myrepublic <nomor>']
handler.tags = ['topup']
handler.command = ['myrepublic']
