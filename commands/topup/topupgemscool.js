...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Gemscool sedang tidak tersedia.`)
        }
...
handler.help = ['topupgemscool', 'gemscool'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topupgemscool', 'gemscool']
