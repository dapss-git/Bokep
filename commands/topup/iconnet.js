...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  I C O N N E T*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *ID Pelanggan* Iconnet.\n\n`
        msg += `*Contoh:* ${usedPrefix + command} 123456789\n`
        return m.reply(msg)
    }
    
    const search = 'iconnet'
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan pembayaran Iconnet sedang tidak tersedia.`)
        }
        ...
        console.error('[ICONNET FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['iconnet <id>']
handler.tags = ['topup']
handler.command = ['iconnet']
