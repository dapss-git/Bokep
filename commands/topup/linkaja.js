...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    const ewallet = 'LINKAJA'
    const typeStr = 'linkaja'
    ...
    try {
        ...
        const filteredData = data.filter(item => {
            ...
            const isMatch = /\blinkaja\b/.test(desc) || /\blink aja\b/.test(desc)
...
handler.help = ['linkaja <nomor>']
handler.tags = ['topup']
handler.command = ['topuplinkaja', 'linkaja']
