...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan Pascabayar sedang tidak tersedia saat ini.`)
        }
...
handler.help = ['pascabayar', 'pasca', 'tagihan'].map(v => v + ' <nomor_tujuan>')
handler.tags = ['topup']
handler.command = ['pascabayar', 'pasca', 'tagihan']
