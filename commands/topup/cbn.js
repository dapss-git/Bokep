...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*P E M B A Y A R A N  T A G I H A N  C B N*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *ID Pelanggan* CBN.\n\n`
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan pembayaran CBN sedang tidak tersedia.`)
        }
