...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Roblox sedang tidak tersedia.`)
        }
...
handler.help = ['topuproblox', 'roblox'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topuproblox', 'roblox']
