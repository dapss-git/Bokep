...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  S T E A M  W A L L E T*\n\n`
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Steam sedang tidak tersedia.`)
        }
...
handler.help = ['topupsteam', 'steam'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topupsteam', 'steam']
