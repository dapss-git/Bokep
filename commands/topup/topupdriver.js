...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  S A L D O  D R I V E R*\n\n`
        msg += `Gunakan perintah ini untuk isi saldo Driver (Grab, Gojek, Maxim, Shopee, dll).\n\n`
        msg += `*Format:* ${usedPrefix + command} <nomor_hp>\n`
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk saldo Driver sedang tidak tersedia.`)
        }
        ...
        console.error('[DRIVER FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['topupdriver <nomor>', 'driver <nomor>']
handler.tags = ['topup']
handler.command = ['topupdriver', 'driver']
