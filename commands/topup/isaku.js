...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        const filteredData = data.filter(item => {
            ...
            const isMatch = /\bisaku\b/.test(desc) || /\bi-saku\b/.test(desc)
            return isMatch && 
        ...
        console.error(`[TOPUP i-Saku FETCH]`, e)
        return m.reply('Sedang gangguan, gagal mengambil daftar harga dari server.')
    }

    const allKeys = Object.keys(packages)
    if (allKeys.length === 0) {
        return m.reply(`Maaf, produk topup i-Saku sedang tidak tersedia atau gangguan dari server.`)
    }
...
handler.help = ['isaku <nomor>']
handler.tags = ['topup']
handler.command = ['topupisaku', 'isaku']
