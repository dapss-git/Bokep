...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan Tagihan PBB sedang tidak tersedia saat ini.`)
        }
...
handler.help = ['pbb', 'pajak', 'bayarpbb'].map(v => v + ' <nomor_tujuan>')
handler.tags = ['topup']
handler.command = ['pbb', 'pajak', 'bayarpbb']
