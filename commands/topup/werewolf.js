...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Werewolf sedang tidak tersedia.`)
        }
        ...
        console.error('[GAME WEREWOLF FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['topupwerewolf', 'werewolf'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topupwerewolf', 'werewolf']
