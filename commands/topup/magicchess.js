...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk Magic Chess sedang tidak tersedia.`)
        }
        ...
        console.error('[MAGICCHESS FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['magicchess <id>', 'topupmc'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['magicchess', 'topupmc']
