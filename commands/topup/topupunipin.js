...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Unipin sedang tidak tersedia.`)
        }
...
handler.help = ['topupunipin', 'unipin'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topupunipin', 'unipin']
