...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  S P O T I F Y  P R E M I U M*\n\n`
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk Spotify sedang tidak tersedia.`)
        }
        ...
        console.error('[SPOTIFY FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['premiumspotify <id>']
handler.tags = ['topup']
handler.command = ['premiumspotify']
