...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  G R A B / O V O*\n\n`
    ...
    try {
        ...
        let products = data.filter(item => {
            ...
            const isDompet = item.kategori === 'DOMPET DIGITAL' || item.kategori === 'DIGITAL'
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk Grab sedang tidak tersedia.`)
        }
        ...
        console.error('[GRAB FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['grab <nomor>']
handler.tags = ['topup']
handler.command = ['grab']
