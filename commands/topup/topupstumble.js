...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  S T U M B L E  G U Y S*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *ID Player* / *Username*.\n\n`
        msg += `*Contoh:* ${usedPrefix + command} 123456789\n`
        return m.reply(msg)
    }
    
    const search = 'stumble'
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Stumble Guys sedang tidak tersedia.`)
        }
        ...
        console.error('[GAME STUMBLE FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['topupstumble', 'stumble'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topupstumble', 'stumble']
