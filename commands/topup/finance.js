...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  F I N A N C E*\n\n`
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan Finance / Cicilan sedang tidak tersedia saat ini.`)
        }
...
handler.help = ['finance', 'kredit', 'cicilan'].map(v => v + ' <nomor_tujuan>')
handler.tags = ['topup']
handler.command = ['finance', 'kredit', 'cicilan']
