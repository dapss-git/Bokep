...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk Maxim sedang tidak tersedia.`)
        }
        ...
        console.error('[MAXIM FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['maxim <id>']
handler.tags = ['topup']
handler.command = ['maxim']
