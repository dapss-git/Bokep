...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*P E M B A Y A R A N  H O M E  C R E D I T*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *Nomor Kontrak* Home Credit.\n\n`
        msg += `*Contoh:* ${usedPrefix + command} 0123456789\n`
        return m.reply(msg)
    }
    
    const search = 'home credit'
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan Home Credit sedang tidak tersedia.`)
        }
        ...
        console.error('[HOMECREDIT FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['homecredit <nomor>']
handler.tags = ['topup']
handler.command = ['homecredit']
