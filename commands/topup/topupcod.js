...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Call of Duty sedang tidak tersedia.`)
        }
...
handler.help = ['topupcod', 'cod', 'codm'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topupcod', 'cod', 'codm']
