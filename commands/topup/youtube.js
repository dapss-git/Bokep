...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  Y O U T U B E  P R E M I U M*\n\n`
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk YouTube sedang tidak tersedia.`)
        }
        ...
        console.error('[YOUTUBE FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['youtube <id>']
handler.tags = ['topup']
handler.command = ['youtube', 'youtubepremium']
