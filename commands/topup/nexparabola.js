...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  N E X  P A R A B O L A*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *SMC ID* Nex Parabola.\n\n`
        msg += `*Contoh:* ${usedPrefix + command} 800123456789\n`
        return m.reply(msg)
    }
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk Nex Parabola sedang tidak tersedia.`)
        }
        ...
        console.error('[NEX FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['nexparabola <id>', 'nex <id>']
handler.tags = ['topup']
handler.command = ['nexparabola', 'nex']
