...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk HBO GO sedang tidak tersedia.`)
        }
        ...
        console.error('[HBO FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['hbogo <id>', 'hbo <id>']
handler.tags = ['topup']
handler.command = ['hbogo', 'hbo']
