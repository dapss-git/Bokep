...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk Kaspro sedang tidak tersedia.`)
        }
        ...
        console.error('[KASPRO FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['kaspro <nomor>']
handler.tags = ['topup']
handler.command = ['kaspro']
