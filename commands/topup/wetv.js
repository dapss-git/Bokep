...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk WeTV sedang tidak tersedia.`)
        }
        ...
        console.error('[WETV FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['wetv <id>']
handler.tags = ['topup']
handler.command = ['wetv']
