...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  H O N O R  O F  K I N G S*\n\n`
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Honor of Kings sedang tidak tersedia.`)
        }
        ...
        console.error('[GAME HOK FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['topuphok', 'hok'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topuphok', 'hok']
