...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  I N D R I V E R*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *ID / Nomor* tujuan.\n\n`
        msg += `*Contoh:* ${usedPrefix + command} 08123456789\n`
        return m.reply(msg)
    }
    
    const search = 'indriver'
    ...
    try {
        ...
        let products = data.filter(item => {
            ...
            const isDigital = item.kategori === 'DOMPET DIGITAL' || item.kategori === 'DIGITAL'
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk InDriver sedang tidak tersedia.`)
        }
        ...
        console.error('[INDRIVER FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['indriver <id>']
handler.tags = ['topup']
handler.command = ['indriver']
