...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk Wifi ID sedang tidak tersedia.`)
        }
        ...
        console.error('[WIFIID FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['wifiid <nomor>']
handler.tags = ['topup']
handler.command = ['wifiid']
