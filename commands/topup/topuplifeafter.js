...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Life After sedang tidak tersedia.`)
        }
...
handler.help = ['topuplifeafter', 'lifeafter'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topuplifeafter', 'lifeafter']
