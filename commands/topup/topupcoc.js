...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Clash of Clans sedang tidak tersedia.`)
        }
        ...
        console.error('[GAME COC FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['topupcoc', 'coc'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topupcoc', 'coc']
