...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  G A R E N A  S H E L L*\n\n`
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Garena sedang tidak tersedia.`)
        }
...
handler.help = ['topupgarena', 'garena', 'shell'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topupgarena', 'garena', 'shell']
