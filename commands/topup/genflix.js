...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        let products = data.filter(item => {
            ...
            const isDigital = item.kategori === 'DIGITAL' || item.kategori.includes('VOUCHER')
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk Genflix sedang tidak tersedia.`)
        }
        ...
        console.error('[GENFLIX FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['genflix <id>']
handler.tags = ['topup']
handler.command = ['genflix']
