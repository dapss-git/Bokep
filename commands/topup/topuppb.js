...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Point Blank sedang tidak tersedia.`)
        }
...
handler.help = ['topuppb', 'pb', 'pointblank'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topuppb', 'pb', 'pointblank']
