...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        const filteredData = data.filter(item => {
            ...
            const isMatch = /\bbrizzi\b/.test(desc) || /\betoll bri\b/.test(desc)
            return isMatch && 
        ...
        console.error(`[TOPUP Brizzi FETCH]`, e)
        return m.reply('Sedang gangguan, gagal mengambil daftar harga dari server.')
    }

    const allKeys = Object.keys(packages)
    if (allKeys.length === 0) {
        return m.reply(`Maaf, produk topup Brizzi sedang tidak tersedia atau gangguan dari server.`)
    }
...
handler.help = ['brizzi <nomor>']
handler.tags = ['topup']
handler.command = ['topupbrizzi', 'brizzi', 'bri']
