...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*P E M B A Y A R A N  I N D I H O M E*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *Nomor Pelanggan* Indihome.\n\n`
        msg += `*Contoh:* ${usedPrefix + command} 123456789\n`
        return m.reply(msg)
    }
    
    const search = 'indihome'
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan pembayaran Indihome sedang tidak tersedia.`)
        }
        ...
        console.error('[INDIHOME FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['indihome <nomor>']
handler.tags = ['topup']
handler.command = ['indihome']
