...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*P E M B A Y A R A N  X L  H O M E*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *Nomor Pelanggan* XL Home.\n\n`
        msg += `*Contoh:* ${usedPrefix + command} 123456789\n`
        return m.reply(msg)
    }
    
    const search = 'xl home'
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan XL Home sedang tidak tersedia.`)
        }
        ...
        console.error('[XLHOME FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['xlhome <nomor>']
handler.tags = ['topup']
handler.command = ['xlhome']
