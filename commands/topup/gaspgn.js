...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*P E M B A Y A R A N  G A S  P G N*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *Nomor Pelanggan* PGN.\n\n`
    ...
    try {
        ...
        let products = data.filter(item => {
            ...
            const isTagihan = item.kategori === 'TAGIHAN' || item.kategori === 'PASCABAYAR'
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan pembayaran Gas PGN sedang tidak tersedia.`)
        }
        ...
        console.error('[GASPGN FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['gaspgn <nomor>', 'pgn <nomor>']
handler.tags = ['topup']
handler.command = ['gaspgn', 'pgn']
