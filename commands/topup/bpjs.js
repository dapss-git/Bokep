...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*P E M B A Y A R A N  B P J S*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *Nomor Pelanggan* BPJS.\n\n`
        msg += `*Contoh:* ${usedPrefix + command} 000123456789\n`
        return m.reply(msg)
    }
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan pembayaran BPJS sedang tidak tersedia.`)
        }
        ...
        console.error('[BPJS FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['bpjs <nomor>']
handler.tags = ['topup']
handler.command = ['bpjs']
