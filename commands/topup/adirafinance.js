...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*P E M B A Y A R A N  A D I R A  F I N A N C E*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *Nomor Kontrak* Adira.\n\n`
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan Adira Finance sedang tidak tersedia.`)
        }
        ...
        console.error('[ADIRA FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['adirafinance <nomor>', 'adira <nomor>']
handler.tags = ['topup']
handler.command = ['adirafinance', 'adira']
