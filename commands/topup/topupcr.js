...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Clash Royale sedang tidak tersedia.`)
        }
        ...
        console.error('[GAME CR FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['topupcr', 'cr'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topupcr', 'cr']
