...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  R A Z E R  G O L D*\n\n`
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk Razer Gold sedang tidak tersedia.`)
        }
        ...
        console.error('[RAZER FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['razer <id>', 'razergold <id>']
handler.tags = ['topup']
handler.command = ['razer', 'razergold']
