...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk FC Mobile sedang tidak tersedia.`)
        }
        ...
        console.error('[GAME FC MOBILE FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['topupfcmobile', 'fcmobile'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topupfcmobile', 'fcmobile']
