...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    const ewallet = 'SHOPEEPAY'
    const typeStr = 'shopeepay'
    ...
    try {
        ...
        const filteredData = data.filter(item => {
            ...
            const isMatch = /\bshopee\b/.test(desc) || /\bshopeepay\b/.test(desc) || /\bspay\b/.test(desc)
...
handler.help = ['shopee <nomor>']
handler.tags = ['topup']
handler.command = ['topupshopee', 'shopeepay', 'shopee', 'spay']
