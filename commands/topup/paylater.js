...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*P E M B A Y A R A N  P A Y L A T E R  /  P I N J O L*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *Nomor HP / ID Pelanggan*.\n\n`
        msg += `*Contoh:* ${usedPrefix + command} 08123456789\n`
        return m.reply(msg)
    }
    
    const keywords = ['akulaku', 'paylater', 'spaylater', 'spinjam', 'julo', 'indodana', 'kredit pintar', 'adakami', 'easycash', 'rupiah cepat']
    ...
    try {
        ...
        let products = data.filter(item => {
            ...
            const txt = (item.keterangan + ' ' + item.produk).toLowerCase()
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan pembayaran Paylater/Pinjol sedang tidak tersedia.`)
        }
        ...
        console.error('[PAYLATER FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['paylater <nomor>', 'pinjol <nomor>']
handler.tags = ['topup']
handler.command = ['paylater', 'pinjol']
