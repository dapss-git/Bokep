...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  K - V I S I O N*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *ID Pelanggan*.\n\n`
        msg += `*Contoh:* ${usedPrefix + command} 123456789\n`
        return m.reply(msg)
    }
    
    const search = 'k-vision'
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk K-Vision sedang tidak tersedia.`)
        }
        ...
        console.error('[KVISION FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['kvision <id>']
handler.tags = ['topup']
handler.command = ['kvision']
