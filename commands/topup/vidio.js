...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk Vidio sedang tidak tersedia.`)
        }
        ...
        console.error('[VIDIO FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['vidio <id>']
handler.tags = ['topup']
handler.command = ['vidio']
