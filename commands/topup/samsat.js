...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*P E M B A Y A R A N  S A M S A T*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *Nomor Rangka / ID Pelanggan* Samsat.\n\n`
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan pembayaran Samsat sedang tidak tersedia.`)
        }
        ...
        if (!planArg) {
            ...
            allKeys.forEach((key, index) => {
                ...
                if (currentRows.length >= 20 || index === allKeys.length - 1) {
                    sections.push({
                        title: `Pilihan Wilayah (Bagian ${sectionIndex++})`,
                        rows: currentRows
                    })
                    currentRows = []
                }
            ...
            await conn.sendMessage(m.chat, {
                text: `*P E M B A Y A R A N  S A M S A T*\n\n◦ *ID / No. Rangka:* ${target}\n◦ *Total Produk:* ${allKeys.length} Pilihan\n\nSilakan pilih wilayah di bawah ini.`,
                footer: global.footer,
                buttons: [{
                    buttonId: "samsat_select",
                    buttonText: { displayText: "🛒 Pilih Wilayah" },
                    type: 4,
                    nativeFlowInfo: {
                        name: "single_select",
                        paramsJson: JSON.stringify({
                            title: `Pembayaran Samsat`,
                            sections: sections
                        })
                    }
                }],
        ...
        console.error('[SAMSAT FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['samsat <nomor>']
handler.tags = ['topup']
handler.command = ['samsat']
