...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk Disney+ sedang tidak tersedia.`)
        }
        ...
        console.error('[DISNEY FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['disney <id>', 'disneyplus <id>']
handler.tags = ['topup']
handler.command = ['disney', 'disneyplus']
