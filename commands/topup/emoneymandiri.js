...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  E - M O N E Y  M A N D I R I*\n\n`
    ...
    try {
        ...
        const filteredData = data.filter(item => {
            ...
            const isMatch = /\bemoney mandiri\b/.test(desc) || /\be-money\b/.test(desc)
            return isMatch && 
        ...
        console.error(`[TOPUP E-Money Mandiri FETCH]`, e)
        return m.reply('Sedang gangguan, gagal mengambil daftar harga dari server.')
    }

    const allKeys = Object.keys(packages)
    if (allKeys.length === 0) {
        return m.reply(`Maaf, produk topup E-Money Mandiri sedang tidak tersedia atau gangguan dari server.`)
    }
...
handler.help = ['emoney <nomor>']
handler.tags = ['topup']
handler.command = ['topupemoney', 'emoney', 'mandiri']
