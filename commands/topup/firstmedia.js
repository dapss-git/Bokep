...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*P E M B A Y A R A N  F I R S T  M E D I A*\n\n`
        msg += `Gunakan perintah ini dengan menyertakan *ID Pelanggan* First Media.\n\n`
        msg += `*Contoh:* ${usedPrefix + command} 123456789\n`
        return m.reply(msg)
    }
    
    const search = 'first media'
    ...
    try {
        ...
        let products = data.filter(item => {
            ...
            const isTagihan = item.kategori === 'TAGIHAN' || item.kategori === 'PASCABAYAR' || item.produk.toLowerCase().includes('internet')
            const matchSearch = item.keterangan.toLowerCase().includes(search) || item.produk.toLowerCase().includes(search)
            return isTagihan && matchSearch
        })
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, layanan pembayaran First Media sedang tidak tersedia.`)
        }
        ...
        console.error('[FIRSTMEDIA FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['firstmedia <nomor>']
handler.tags = ['topup']
handler.command = ['firstmedia']
