...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Zepeto sedang tidak tersedia.`)
        }
        ...
        console.error('[GAME ZEPETO FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['topupzepeto', 'zepeto'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topupzepeto', 'zepeto']
