...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    if (!target) {
        let msg = `*T O P U P  V A L O R A N T*\n\n`
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk game untuk Valorant sedang tidak tersedia.`)
        }
...
handler.help = ['topupvalo', 'valo', 'valorant'].map(v => v + ' <id>')
handler.tags = ['topup']
handler.command = ['topupvalo', 'valo', 'valorant']
