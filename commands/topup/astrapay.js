...
const handler = async (m, { conn, text, usedPrefix, command, Func }) => {
    ...
    try {
        ...
        if (allKeys.length === 0) {
            return m.reply(`Maaf, produk Astrapay sedang tidak tersedia.`)
        }
        ...
        console.error('[ASTRAPAY FETCH]', e)
        m.reply('Sedang gangguan, gagal mengambil data dari server.')
    }
}

handler.help = ['astrapay <nomor>']
handler.tags = ['topup']
handler.command = ['astrapay']
