...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    try {
        let api = global.API("theresav", "/berita/google", { q: text }, "apikey");
        ...
        let linkPreview = img ? await makeLinkPreview(conn, img, headline.title, "Google News", headline.link || headline.url) : null;
...
handler.help = ["gnews <query>"];
handler.tags = ["berita"];
handler.command = /^gnews$/i;
