let handler = async (m, {
    ...
    let lastngojek = global.db.data.users[m.sender].lastngojek || 0;
    let __timers = (new Date - lastngojek);
    let _timers = Math.max(0, 300000 - __timers); // 5-minute cooldown
    let order = global.db.data.users[m.sender].ojekk || 0;
    let timers = clockString(_timers);
    let user = global.db.data.users[m.sender];
    let name = user.name || m.sender.split('@')[0];

    if (new Date - lastngojek > 300000) {
        ...
        Dosa Bertambah: +1
        Total Dosa: ${(global.db.data.users[m.sender].dosa || 0) + 1}
        `;
        ...
        global.db.data.users[m.sender].dosa = (global.db.data.users[m.sender].dosa || 0) + 1;

        // Send initial message and store its key
        let initialMessage = await conn.sendMessage(m.chat, { text: 'Mencari Pelanggan.....' }, { quoted: m });
