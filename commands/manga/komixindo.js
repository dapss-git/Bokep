...
let handler = async (m, { args, conn }) => {
    ...
    switch (subcommand) {
        case 'search': {
            ...
            try {
                const url = global.API('theresav', '/manga/komixindo/search', { q: keyword }, 'apikey');
                ...
                let rows = results.slice(0, 50).map(v => ({
                    ...
                    id: `.komixindo detail ${v.id || v.url || v.slug}`
                }));

                await conn.sendMessage(m.chat, {
                    text: `Hasil pencarian komixindo: *${keyword}*`,
                ...
                console.error('KOMIXINDO SEARCH ERROR:', e?.response?.data || e.message);
        ...
        case 'latest':
        case 'home': {
            await m.reply('Mengambil update terbaru dari komixindo...');
            try {
                const url = global.API('theresav', '/manga/komixindo/homepage', {}, 'apikey');
                const { data } = await axios.get(url);
                const results = data?.data || data?.result || data?.results;
                ...
                console.error('KOMIXINDO HOME ERROR:', e?.response?.data || e.message);
        ...
        case 'detail': {
            ...
            try {
                const params = {};
                params['url'] = val;
                const apiUrl = global.API('theresav', '/manga/komixindo/detail', params, 'apikey');
                ...
                const rows = chapters.map((ch, idx) => ({
                    id: `.komixindo download ${ch.url || ch.link || ch.id || ch.slug || ch.chapterId}`,
                ...
                console.error('KOMIXINDO DETAIL ERROR:', e?.response?.data || e.message);
        ...
        case 'download': {
            ...
            try {
                const params = {};
                let paramKey = 'url';
                if ('komixindo' === 'manta' || 'komixindo' === 'mangadex') paramKey = 'id';
                else if ('url' === 'slug' && !val.startsWith('http')) paramKey = 'slug';
                
                params[paramKey] = val;

                const apiUrl = global.API('theresav', '/manga/komixindo/download', params, 'apikey');
                const { data } = await axios.get(apiUrl);
                ...
                const cleanName = `komixindo_chapter`.replace(/[^\w\s-]/gi, '').replace(/\s+/g, '-');
                const pdfPath = `${TMP_DIR}/${Date.now()}_komixindo.pdf`;
                const imgPaths = [];
                const limit = 5;
                ...
                for (let i = 0; i < images.length; i += limit) {
                    ...
                    results.forEach(r => {
                        ...
                        else console.error(`[DL komixindo] Failed:`, r.reason?.message);
                    });
                }
                ...
                console.error('KOMIXINDO DOWNLOAD ERROR:', e?.response?.data || e.message);
        ...
        default:
            m.reply(
                `*KOMIXINDO*\n\n` +
                `.komixindo search <judul>\n` +
                `.komixindo latest\n` +
                `.komixindo detail <url/id>\n` +
                `.komixindo download <url/id>`
            );
    }
};

handler.command = /^(komixindo)$/i;
handler.help = [
    'komixindo search <judul>',
    'komixindo latest',
    'komixindo detail <url>',
    'komixindo download <url>'
].filter(Boolean);
handler.tags = ['manga'];
handler.limit = true;
