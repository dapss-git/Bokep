...
let handler = async (m, { args, conn }) => {
    ...
    switch (subcommand) {
        case 'search': {
            ...
            try {
                const url = global.API('theresav', '/manga/soulscans/search', { q: keyword }, 'apikey');
                ...
                let rows = results.slice(0, 50).map(v => ({
                    ...
                    id: `.soulscans detail ${v.id || v.url || v.slug}`
                }));

                await conn.sendMessage(m.chat, {
                    text: `Hasil pencarian soulscans: *${keyword}*`,
                ...
                console.error('SOULSCANS SEARCH ERROR:', e?.response?.data || e.message);
        ...
        case 'latest':
        case 'home': {
            await m.reply('Mengambil update terbaru dari soulscans...');
            try {
                const url = global.API('theresav', '/manga/soulscans/latest', {}, 'apikey');
                const { data } = await axios.get(url);
                const results = data?.data || data?.result || data?.results;
                ...
                console.error('SOULSCANS HOME ERROR:', e?.response?.data || e.message);
        ...
        case 'detail': {
            ...
            try {
                const params = {};
                params['url'] = val;
                const apiUrl = global.API('theresav', '/manga/soulscans/detail', params, 'apikey');
                ...
                const rows = chapters.map((ch, idx) => ({
                    id: `.soulscans download ${ch.url || ch.link || ch.id || ch.slug || ch.chapterId}`,
                ...
                console.error('SOULSCANS DETAIL ERROR:', e?.response?.data || e.message);
        ...
        case 'download': {
            ...
            try {
                const params = {};
                let paramKey = 'url';
                if ('soulscans' === 'manta' || 'soulscans' === 'mangadex') paramKey = 'id';
                else if ('url' === 'slug' && !val.startsWith('http')) paramKey = 'slug';
                
                params[paramKey] = val;

                const apiUrl = global.API('theresav', '/manga/soulscans/download', params, 'apikey');
                const { data } = await axios.get(apiUrl);
                ...
                const cleanName = `soulscans_chapter`.replace(/[^\w\s-]/gi, '').replace(/\s+/g, '-');
                const pdfPath = `${TMP_DIR}/${Date.now()}_soulscans.pdf`;
                const imgPaths = [];
                const limit = 5;
                ...
                for (let i = 0; i < images.length; i += limit) {
                    ...
                    results.forEach(r => {
                        ...
                        else console.error(`[DL soulscans] Failed:`, r.reason?.message);
                    });
                }
                ...
                console.error('SOULSCANS DOWNLOAD ERROR:', e?.response?.data || e.message);
        ...
        default:
            m.reply(
                `*SOULSCANS*\n\n` +
                `.soulscans search <judul>\n` +
                `.soulscans latest\n` +
                `.soulscans detail <url/id>\n` +
                `.soulscans download <url/id>`
            );
    }
};

handler.command = /^(soulscans)$/i;
handler.help = [
    'soulscans search <judul>',
    'soulscans latest',
