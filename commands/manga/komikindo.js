...
let handler = async (m, { args, conn }) => {
    ...
    switch (subcommand) {
        case 'search': {
            ...
            try {
                const url = global.API('theresav', '/manga/komikindo/search', { q: keyword }, 'apikey');
                ...
                let rows = results.slice(0, 50).map(v => ({
                    ...
                    id: `.komikindo detail ${v.id || v.url || v.slug}`
                }));

                await conn.sendMessage(m.chat, {
                    text: `Hasil pencarian komikindo: *${keyword}*`,
                ...
                console.error('KOMIKINDO SEARCH ERROR:', e?.response?.data || e.message);
        ...
        case 'latest':
        case 'home': {
            await m.reply('Mengambil update terbaru dari komikindo...');
            try {
                const url = global.API('theresav', '/manga/komikindo/latest', {}, 'apikey');
                const { data } = await axios.get(url);
                const results = data?.data || data?.result || data?.results;
                ...
                console.error('KOMIKINDO HOME ERROR:', e?.response?.data || e.message);
        ...
        case 'detail': {
            ...
            try {
                const params = {};
                params['url'] = val;
                const apiUrl = global.API('theresav', '/manga/komikindo/info', params, 'apikey');
                ...
                const rows = chapters.map((ch, idx) => ({
                    id: `.komikindo download ${ch.url || ch.link || ch.id || ch.slug || ch.chapterId}`,
                ...
                console.error('KOMIKINDO DETAIL ERROR:', e?.response?.data || e.message);
        ...
        case 'download': {
            ...
            try {
                const params = {};
                let paramKey = 'url';
                if ('komikindo' === 'manta' || 'komikindo' === 'mangadex') paramKey = 'id';
                else if ('url' === 'slug' && !val.startsWith('http')) paramKey = 'slug';
                
                params[paramKey] = val;

                const apiUrl = global.API('theresav', '/manga/komikindo/download', params, 'apikey');
                const { data } = await axios.get(apiUrl);
                ...
                const cleanName = `komikindo_chapter`.replace(/[^\w\s-]/gi, '').replace(/\s+/g, '-');
                const pdfPath = `${TMP_DIR}/${Date.now()}_komikindo.pdf`;
                const imgPaths = [];
                const limit = 5;
                ...
                for (let i = 0; i < images.length; i += limit) {
                    ...
                    results.forEach(r => {
                        ...
                        else console.error(`[DL komikindo] Failed:`, r.reason?.message);
                    });
                }
                ...
                console.error('KOMIKINDO DOWNLOAD ERROR:', e?.response?.data || e.message);
        ...
        default:
            m.reply(
                `*KOMIKINDO*\n\n` +
                `.komikindo search <judul>\n` +
                `.komikindo latest\n` +
                `.komikindo detail <url/id>\n` +
                `.komikindo download <url/id>`
            );
    }
};
...
handler.help = [
    'komikindo search <judul>',
    'komikindo latest',
    'komikindo detail <url>',
    'komikindo download <url>'
].filter(Boolean);
handler.tags = ['manga'];
handler.limit = true;
