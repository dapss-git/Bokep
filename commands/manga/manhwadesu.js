...
let handler = async (m, { args, conn }) => {
    ...
    switch (subcommand) {
        case 'search': {
            ...
            try {
                const url = global.API('theresav', '/manga/manhwadesu/search', { q: keyword }, 'apikey');
                ...
                let rows = results.slice(0, 50).map(v => ({
                    ...
                    id: `.manhwadesu detail ${v.id || v.url || v.slug}`
                }));

                await conn.sendMessage(m.chat, {
                    text: `Hasil pencarian manhwadesu: *${keyword}*`,
                ...
                console.error('MANHWADESU SEARCH ERROR:', e?.response?.data || e.message);
        ...
        case 'latest':
        case 'home': {
            await m.reply('Mengambil update terbaru dari manhwadesu...');
            try {
                const url = global.API('theresav', '/manga/manhwadesu/latest', {}, 'apikey');
                const { data } = await axios.get(url);
                const results = data?.data || data?.result || data?.results;
                ...
                await conn.sendMessage(m.chat, {
                    text: `Update Terbaru manhwadesu 🆕`,
                ...
                console.error('MANHWADESU HOME ERROR:', e?.response?.data || e.message);
        ...
        case 'detail': {
            ...
            try {
                const params = {};
                params['url'] = val;
                const apiUrl = global.API('theresav', '/manga/manhwadesu/detail', params, 'apikey');
                ...
                const rows = chapters.map((ch, idx) => ({
                    id: `.manhwadesu download ${ch.url || ch.link || ch.id || ch.slug || ch.chapterId}`,
                ...
                console.error('MANHWADESU DETAIL ERROR:', e?.response?.data || e.message);
        ...
        case 'download': {
            ...
            try {
                const params = {};
                let paramKey = 'url';
                if ('manhwadesu' === 'manta' || 'manhwadesu' === 'mangadex') paramKey = 'id';
                else if ('url' === 'slug' && !val.startsWith('http')) paramKey = 'slug';
                
                params[paramKey] = val;

                const apiUrl = global.API('theresav', '/manga/manhwadesu/chapter', params, 'apikey');
                const { data } = await axios.get(apiUrl);
                ...
                const cleanName = `manhwadesu_chapter`.replace(/[^\w\s-]/gi, '').replace(/\s+/g, '-');
                const pdfPath = `${TMP_DIR}/${Date.now()}_manhwadesu.pdf`;
                const imgPaths = [];
                const limit = 5;
                ...
                for (let i = 0; i < images.length; i += limit) {
                    ...
                    results.forEach(r => {
                        ...
                        else console.error(`[DL manhwadesu] Failed:`, r.reason?.message);
                    });
                }
                ...
                console.error('MANHWADESU DOWNLOAD ERROR:', e?.response?.data || e.message);
        ...
        default:
            m.reply(
                `*MANHWADESU*\n\n` +
                `.manhwadesu search <judul>\n` +
                `.manhwadesu latest\n` +
                `.manhwadesu detail <url/id>\n` +
                `.manhwadesu download <url/id>`
            );
    }
};

handler.command = /^(manhwadesu)$/i;
handler.help = [
    'manhwadesu search <judul>',
    'manhwadesu latest',
    'manhwadesu detail <url>',
    'manhwadesu download <url>'
].filter(Boolean);
handler.tags = ['manga'];
handler.limit = true;
