...
let handler = async (m, { args, conn }) => {
    ...
    switch (subcommand) {
        case 'search': {
            ...
            try {
                const url = global.API('theresav', '/manga/mangapoi/search', { q: keyword }, 'apikey');
                ...
                let rows = results.slice(0, 50).map(v => ({
                    ...
                    id: `.mangapoi detail ${v.id || v.url || v.slug}`
                }));

                await conn.sendMessage(m.chat, {
                    text: `Hasil pencarian mangapoi: *${keyword}*`,
                ...
                console.error('MANGAPOI SEARCH ERROR:', e?.response?.data || e.message);
        ...
        case 'latest':
        case 'home': {
            await m.reply('Mengambil update terbaru dari mangapoi...');
            try {
                const url = global.API('theresav', '/manga/mangapoi/latest', {}, 'apikey');
                const { data } = await axios.get(url);
                const results = data?.data || data?.result || data?.results;
                ...
                console.error('MANGAPOI HOME ERROR:', e?.response?.data || e.message);
        ...
        case 'detail': {
            ...
            try {
                const params = {};
                params['url'] = val;
                const apiUrl = global.API('theresav', '/manga/mangapoi/detail', params, 'apikey');
                ...
                const rows = chapters.map((ch, idx) => ({
                    id: `.mangapoi download ${ch.url || ch.link || ch.id || ch.slug || ch.chapterId}`,
                ...
                console.error('MANGAPOI DETAIL ERROR:', e?.response?.data || e.message);
        ...
        case 'download': {
            ...
            try {
                const params = {};
                let paramKey = 'url';
                if ('mangapoi' === 'manta' || 'mangapoi' === 'mangadex') paramKey = 'id';
                else if ('url' === 'slug' && !val.startsWith('http')) paramKey = 'slug';
                
                params[paramKey] = val;

                const apiUrl = global.API('theresav', '/manga/mangapoi/chapter', params, 'apikey');
                const { data } = await axios.get(apiUrl);
                ...
                const cleanName = `mangapoi_chapter`.replace(/[^\w\s-]/gi, '').replace(/\s+/g, '-');
                const pdfPath = `${TMP_DIR}/${Date.now()}_mangapoi.pdf`;
                const imgPaths = [];
                const limit = 5;
                ...
                for (let i = 0; i < images.length; i += limit) {
                    ...
                    results.forEach(r => {
                        ...
                        else console.error(`[DL mangapoi] Failed:`, r.reason?.message);
                    });
                }
                ...
                console.error('MANGAPOI DOWNLOAD ERROR:', e?.response?.data || e.message);
        ...
        default:
            m.reply(
                `*MANGAPOI*\n\n` +
                `.mangapoi search <judul>\n` +
                `.mangapoi latest\n` +
                `.mangapoi detail <url/id>\n` +
                `.mangapoi download <url/id>`
            );
    }
};

handler.command = /^(mangapoi)$/i;
handler.help = [
    'mangapoi search <judul>',
    'mangapoi latest',
    'mangapoi detail <url>',
    'mangapoi download <url>'
].filter(Boolean);
handler.tags = ['manga'];
handler.limit = true;
