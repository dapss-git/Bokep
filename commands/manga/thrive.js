...
let handler = async (m, { args, conn }) => {
    ...
    switch (subcommand) {
        case 'search': {
            ...
            try {
                const url = global.API('theresav', '/manga/thrive/search', { q: keyword }, 'apikey');
                ...
                let rows = results.slice(0, 50).map(v => ({
                    ...
                    id: `.thrive detail ${v.id || v.url || v.slug}`
                }));

                await conn.sendMessage(m.chat, {
                    text: `Hasil pencarian thrive: *${keyword}*`,
                ...
                console.error('THRIVE SEARCH ERROR:', e?.response?.data || e.message);
        ...
        case 'latest':
        case 'home': {
            await m.reply('Mengambil update terbaru dari thrive...');
            try {
                const url = global.API('theresav', '/manga/thrive/home', {}, 'apikey');
                const { data } = await axios.get(url);
                const results = data?.data || data?.result || data?.results;
                ...
                console.error('THRIVE HOME ERROR:', e?.response?.data || e.message);
        ...
        case 'detail': {
            ...
            try {
                const params = {};
                params['url'] = val;
                const apiUrl = global.API('theresav', '/manga/thrive/detail', params, 'apikey');
                ...
                const rows = chapters.map((ch, idx) => ({
                    id: `.thrive download ${ch.url || ch.link || ch.id || ch.slug || ch.chapterId}`,
                ...
                console.error('THRIVE DETAIL ERROR:', e?.response?.data || e.message);
        ...
        case 'download': {
            ...
            try {
                const params = {};
                let paramKey = 'url';
                if ('thrive' === 'manta' || 'thrive' === 'mangadex') paramKey = 'id';
                else if ('url' === 'slug' && !val.startsWith('http')) paramKey = 'slug';
                
                params[paramKey] = val;

                const apiUrl = global.API('theresav', '/manga/thrive/chapter', params, 'apikey');
                const { data } = await axios.get(apiUrl);
                ...
                const cleanName = `thrive_chapter`.replace(/[^\w\s-]/gi, '').replace(/\s+/g, '-');
                const pdfPath = `${TMP_DIR}/${Date.now()}_thrive.pdf`;
                const imgPaths = [];
                const limit = 5;
                ...
                for (let i = 0; i < images.length; i += limit) {
                    ...
                    results.forEach(r => {
                        ...
                        else console.error(`[DL thrive] Failed:`, r.reason?.message);
                    });
                }
                ...
                console.error('THRIVE DOWNLOAD ERROR:', e?.response?.data || e.message);
        ...
        default:
            m.reply(
                ...
                `.thrive search <judul>\n` +
                `.thrive latest\n` +
                `.thrive detail <url/id>\n` +
                `.thrive download <url/id>`
            );
    }
};

handler.command = /^(thrive)$/i;
handler.help = [
    'thrive search <judul>',
    'thrive latest',
    'thrive detail <url>',
