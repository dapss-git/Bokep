...
function getStickerStore(sender) {
    ...
    return global.db.data.sticker[sender]
}
    ...
    await conn.relayMessage(
        m.chat, {
            ...
            stickerPackMessage: {
                ...
                packDescription: `Stiker pack ini di buat menggunakan ${global.wm}`,
...
const handler = async (m, {
    ...
    if (command === 'tspk') {
        if (!pack.length)
            return m.reply('Pack masih kosong, tambahkan dulu dengan .addtspk')

        return sendCustomStickerPack(conn, m, pack)
    }

    if (!m.quoted || !m.quoted.fakeObj.message?.stickerMessage)
        return m.reply('Reply sticker yang mau diproses.')

    const sticker = m.quoted.fakeObj.message.stickerMessage
    ...
    if (command === 'addtspk') {
        ...
        return m.reply(`Ditambahkan (${label}). Total ${pack.length} sticker di pack.`)
    }
...
handler.command = ['tspk', 'addtspk', 'deltspk']
handler.tags = ['maker']
