...
let handler = async (m, {
    ...
    if (!text) return m.reply(`Contoh: ${usedPrefix + command} BIG MONEY NEVER COMES CLEAN 💸💸`)

    // Parse optional format: tema|teks or blur:tema|teks
    // Simple: split by '|' -> first part is options, second is text
    let parts = text.split('|')
    let options = {}
    let teks = ''
    if (parts.length >= 2) {
        const opts = parts[0].trim().toLowerCase()
        teks = parts.slice(1).join('|').trim()
        const optMap = Object.fromEntries(opts.split(',').map(o => o.trim().split('=')))
        if (optMap['tema']) options.theme = optMap['tema']
        if (optMap['blur']) options.blur = parseInt(optMap['blur']) || 0
        if (optMap['format']) options.format = optMap['format']
        if (optMap['hold']) options.holdDuration = parseFloat(optMap['hold']) || 1.5
    } else {
        teks = text.trim()
    }
    ...
    const holdDuration = options.holdDuration ?? 1.5
    const fastProgress = true
    ...
    try {
        ...
        const fileBuffer = await (await import('fs/promises')).readFile(outPath)
        ...
        await conn.sendMessage(m.chat, {
            ...
            gifPlayback: isGif,
            caption: `✅ Brat video ${format.toUpperCase()} selesai`
        }, {
            quoted: m
        })

        // Bersihkan file output
        await (await import('fs/promises')).unlink(outPath).catch(() => {})
    } catch (e) {
        console.error(e)
...
handler.tags = ['maker']
handler.command = /^(bratvideo2|bratvid2)$/i
handler.limit = true
