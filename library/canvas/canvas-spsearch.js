...
export async function canvas(tracks, query) {
    ...
    const totalArtists = new Set(tracks.slice(0, 20).map(t => t.artists || t.artist)).size;
    ...
    const statsCards = [
        ...
        {
            title: "ARTISTS",
            value: new Set(tracks.slice(0, 20).map(t => t.artists || t.artist)).size.toString(),
        },
    ];
    ...
    for (let i = 0; i < displayTracks.length && i < 9; i++) {
        ...
        let title = track.name || track.title || "Unknown";
        ...
        ctx.fillText(track.artists || track.artist || "Unknown", x + 20, infoY + 32);
    ...
    ctx.font = "12px Cobbler";
    ctx.fillText(`${tracks.length} tracks • ${new Set(tracks.slice(0, 20).map(t => t.artists || t.artist)).size} artists`, BASE_WIDTH / 2, footerY + 50);
